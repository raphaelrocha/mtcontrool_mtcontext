...
class App extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, description, developer', 'required'),
                        array('name','unique','message'=>"App already exists!"),
			...
			array('id, name, description, developer', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'platforms' => array(self::MANY_MANY, 'Platforms', 'app_platform(id_app, id_platform)'),
                       ...
                       // 'appUsers'=>array(self::HAS_MANY,'AppUsers','app_users(id_app, id_users)'),
			'runs' => array(self::HAS_MANY, 'Runs', 'id_app'),
                        'users' => array(self::MANY_MANY, 'Users', 'app_users(id_app,id_users)'),
		);
	}
	...
	public static function model($className=__CLASS__)
	{
       ...
       public static function getApp(){
                ...
                $connection=Yii::app()->db;
                $MYTEST = " SELECT DISTINCT a.id, a.name FROM app as a LEFT JOIN app_users
                            ON a.id = app_users.id_app
                            WHERE app_users.id_users = ".$user_Id;
                $commandoteste=$connection->createCommand($MYTEST);
                $tentativateste=$commandoteste->query();

             /*   foreach ($tentativa as $vamo){
                    $value[] = $vamo['name'];
                }*/
                
                $data2 = CHtml::listData($tentativateste, 'id', 'name');
